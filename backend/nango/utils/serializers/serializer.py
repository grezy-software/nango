...
from nango.utils.serializers.fields import Field
...
class MetaSerializer(type):
    ...
    def __new__(cls: MetaSerializer, name: str, bases: tuple, attrs: dict) -> Serializer:
        ...
        Args:
            ...
            attrs: Method and attributes of the class (like Fields).
        """
        fields = {}

        # Get direct Fields from attributes
        fields = {key: value for key, value in attrs.items() if isinstance(value, Field)}
        for key in fields:
            del attrs[key]

        # Make serializer class
        serializer_cls = super(MetaSerializer, cls).__new__(cls, name, bases, attrs)  # noqa: UP008
...
class Serializer(BaseSerializer, Field, metaclass=MetaSerializer):  # noqa
    ...
    def __init__(self, instance=None, data=None, many=False, **kwargs):  # noqa
        ...
        # To use a serializer as a field into another serializer
        super(Field, self).__init__(**kwargs)

        # Save additional kwargs for compatibility
        self._kwargs = kwargs
    ...
    def validated_data(self):  # noqa
        if self._validated_data is None:
            error_msg: str = "Validate data before calling the validated_data property."
    ...
    def validate_data(self, data: dict) -> any:
        ...
        validated_data = {}
        for name, (required, validator, read_only) in self.validators_map.items():
            if read_only:
                error_msg: str = "You cannot validate a read_only field: {name}"
                raise ValidationError(error_msg)

            elt = data.get(name)

            # If content is None, there is nothing to validate
            if elt is None:
                if required:
    ...
    def is_valid(self, **kwargs: dict) -> bool:  # noqa: ARG002
        """Mocked method for compatibility with drf.serializers.Serializer."""
        return True
